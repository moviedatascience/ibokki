...
|---|---|---|---|---|---|
| 1 | **Board background (table felt) — center is an ARENA, not a circle** (direction 2026-07-09: "the middle is the arena space for the two wizards to face off — no circle in it"; three authored circle variants rejected, see `art/review/wave2/` + `chosen.json`) | `PixiBoard.ts` `buildStatic()` — `bgLayer` Container is ready and empty; stack resolves at world (640,300) in a 1280×800 world; `flashStack()` gold pulse overlays it | Flat `#0e1411` + 3 stroked placeholder circles (now known-wrong) | Full-bleed 2560×1600 (2× world, 16:10) composing the center as a dueling arena | 1 |
